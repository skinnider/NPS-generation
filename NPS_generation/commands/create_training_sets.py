...
def add_args(parser):
    ...
    parser.add_argument(
        "--train-file", type=str, required=True, help="Output training smiles file path ({fold} in path is populated automatically)"
    )
    parser.add_argument(
        "--vocab-file", type=str, required=True, help="Output training smiles vocabulary file path ({fold} in path is populated automatically)"
    )
    parser.add_argument(
        "--test-file", type=str, required=True, help="Output test smiles file path ({fold} in path is populated automatically)"
    )
    parser.add_argument(
        "--enum-factor", type=int, default=0, help="For each input smile, number of randomized smiles to generate (0 for no augmentation)"
    )
    parser.add_argument(
        "--folds", type=int, default=10, help="Number of CV Folds to generate for train/test split (default %(default)s, 0 to not generate test data)"
    )
    parser.add_argument(
        "--representation", type=str, default="SMILES", help="Representation (one of SMILES/SELFIES)"
    )
    parser.add_argument(
        "--min-tc", type=float, default=0, help="Minimum fingerprint similarity (Tanimoto Coefficient) to seed molecule. 0 for no similarity requirement"
    )
    parser.add_argument(
        "--n-molecules", type=int, default=100, help="Number of molecules to generate for each seed molecule"
    )
    parser.add_argument(
...
def get_similar_smiles(input_smiles, min_tc, n_molecules=100, max_tries=200, seed=None):
    ...
    if not success:
        raise RuntimeError(f"Unable to pick {n_molecules} molecules with min_tc={min_tc}")

    return subset_smiles


def create_training_sets(input_file=None, train_file=None, test_file=None, vocab_file=None, folds=10, enum_factor=0, representation='SMILES', min_tc=0, n_molecules=100, max_tries=200, seed=None, max_input_smiles=None):
    ...
    if min_tc > 0:
        ...
        smiles = get_similar_smiles(smiles, min_tc=min_tc, n_molecules=n_molecules, max_tries=max_tries)
    ...
    for fold in range(len(folds)):
        ...
        logger.info(
            "vocabulary of {} characters".format(len(vocabulary)))
