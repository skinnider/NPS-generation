...
def add_args(parser):
    ...
    parser.add_argument(
        "--min-heavy-atoms", type=int, default=3, help="Minimum number of heavy atoms that a valid molecule should have"
    )
    parser.add_argument(
        "--no-neutralise", action="store_true", help="Do not neutralise charges (default False)"
    )
    parser.add_argument("--valid-atoms", nargs='+', help='Elements for valid molecules (default %(default)s)',
                        default=VALID_ELEMENTS)
    parser.add_argument(
        "--remove-rare", action="store_true", help="Remove molecules with tokens found in <0.01% or <10 molecules"
    )
    parser.add_argument(
        "--max-input-smiles", type=int, default=None, help="Maximum input smiles to read (useful for testing)"
    )

    return parser


def preprocess(input_file, output_file, max_input_smiles=None, neutralise=True, min_heavy_atoms=3, valid_atoms=None, remove_rare=False, chunk_size=100000):
    ...
    def preprocess_chunk(input_smiles, neutralise=True, min_heavy_atoms=3, valid_atoms=None):
        ...
        if min_heavy_atoms > 0:
            mols = [remove_salts_solvents(mol, hac=min_heavy_atoms) if mol else None for mol in mols]
