...
res = res.append(pd.DataFrame({"outcome": "Molecular weight", "value": mws}))
res = res.append(pd.DataFrame({"outcome": "LogP", "value": logp}))
res = res.append(pd.DataFrame({"outcome": "BertzTC", "value": tcs}))
res = res.append(pd.DataFrame({"outcome": "TPSA", "value": tpsa}))
res = res.append(pd.DataFrame({"outcome": "QED", "value": qed}))
res = res.append(pd.DataFrame({"outcome": "% sp3 carbons", "value": pct_sp3}))
res = res.append(pd.DataFrame({"outcome": "% rotatable bonds", "value": pct_rot}))
res = res.append(pd.DataFrame({"outcome": "% stereocentres", "value": pct_stereo}))
res = res.append(pd.DataFrame({"outcome": "% heteroatoms", "value": pct_hetero}))
res = res.append(pd.DataFrame({"outcome": "# of rings", "value": rings}))
res = res.append(pd.DataFrame({"outcome": "# of aliphatic rings", "value": ali_rings}))
res = res.append(pd.DataFrame({"outcome": "# of aromatic rings", "value": aro_rings}))
res = res.append(pd.DataFrame({"outcome": "# of hydrogen donors", "value": h_donors}))
res = res.append(
    pd.DataFrame({"outcome": "# of hydrogen acceptors", "value": h_acceptors})
)
res = res.append(
    pd.DataFrame({"outcome": "Synthetic accessibility score", "value": SA})
)
res = res.append(
    pd.DataFrame({"outcome": "Natural product-likeness score", "value": NP})
)
...
for idx, scaffold in enumerate(murcko_counts[0]):
    ...
    res = res.append(
        pd.DataFrame(
            {"outcome": "Murcko scaffold: " + scaffold, "value": [murcko_count]}
        )
    )
