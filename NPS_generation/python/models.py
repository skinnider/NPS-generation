...
class RNN(nn.Module):
    ...
    def sample(self, n_sequences, max_len=250, return_smiles=True, 
               return_losses=False):
        ...
        inputs = torch.empty(n_sequences).fill_(start_token).long().\
            view(1, n_sequences).to(self.device)
        # initialize hidden state
        if self.rnn_type == 'LSTM':
            hidden = torch.zeros(self.n_layers, n_sequences, 
                                 self.hidden_size).to(self.device), \
                     torch.zeros(self.n_layers, n_sequences, 
                                 self.hidden_size).to(self.device)
        else:
            hidden = torch.zeros(self.n_layers, n_sequences, 
                                 self.hidden_size).to(self.device)
        ...
        for step in range(max_len):
            ...
            inputs = torch.multinomial(prob.squeeze(0), num_samples=1).\
                view(1, -1)
        ...
        if return_smiles:
            outputs = [self.vocabulary.decode(seq.cpu().numpy()) for \
                      seq in seqs]
        else:
            outputs = sequences
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, 
        ...
        self.c_attn = nn.Linear(self.embedding_size, 
        ...
        if not self.flash:
            ...
            self.register_buffer("bias", torch.\
                                 tril(torch.ones(self.max_len, self.max_len)).\
    ...
    def forward(self, x):
        ...
        if self.flash:
            ...
            y = torch.nn.functional.\
                scaled_dot_product_attention(q, k, v, attn_mask=None, 
                                             dropout_p=self.dropout if \
...
class MLP(nn.Module):
    def __init__(self, 
        ...
        self.dropout = dropout
        self.bias = bias
        self.c_fc    = nn.Linear(self.embedding_size, 
                                 self.exp_factor * self.embedding_size, 
                                 bias=self.bias)
        self.gelu    = nn.GELU()
        self.c_proj  = nn.Linear(self.exp_factor * self.embedding_size,
                                 self.embedding_size, 
                                 bias=self.bias)
...
class Transformer(nn.Module):
    def __init__(self,
        ...
        self.transformer = nn.ModuleDict(dict(
            wte = nn.Embedding(self.vocabulary_size, self.embedding_size,
                               padding_idx=padding_t),
            wpe = nn.Embedding(self.max_len, self.embedding_size),
            drop = nn.Dropout(self.dropout),
            h = nn.ModuleList([Block(embedding_size=self.embedding_size,
                 max_len=self.max_len,
                 n_heads=self.n_heads,
                 exp_factor=self.exp_factor,
                 dropout=self.dropout,
                 bias=self.bias) for _ in range(self.n_blocks)]),
            ln_f = LayerNorm(self.embedding_size, bias=self.bias),
        ))
        self.lm_head = nn.Linear(self.embedding_size, 
    ...
    def sample(self, n_sequences, return_smiles=True, return_losses=False):
        ...
        # create start token tensor
        inputs = torch.empty(n_sequences).fill_(start_token).long().\
            view(n_sequences, 1).to(self.device)
