...
# parse arguments
parser = ArgumentParser(prog = 'outer-calculate-effect-sizes-transformer.R')
...
sh_file = 'sh/break-SELFIES/calculate-effect-sizes-transformer.sh'
sh_dir = dirname(sh_file)
if (!dir.exists(sh_dir)) 
  dir.create(sh_dir, recursive = TRUE)
write_sh(job_name = 'calculate-effect-sizes-transformer',
         sh_file = sh_file,
         grid_file = grid_file,
         inner_file = 'R/break-SELFIES/inner-calculate-effect-sizes.R',
         system = system,
         time = 3,
         mem = 32)
