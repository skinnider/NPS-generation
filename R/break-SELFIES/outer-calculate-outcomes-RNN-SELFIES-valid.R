# Calculate outcomes for recurrent neural network-based models of SELFIES 
# strings that have been decoded without valency constraints. 
setwd("~/git/invalid-smiles-analysis")
options(stringsAsFactors = FALSE)
library(argparse)

# parse arguments
parser = ArgumentParser(prog = 'outer-calculate-outcomes-RNN-SELFIES-valid.R')
...
sh_file = 'sh/calculate-outcomes/calculate-outcomes-RNN-SELFIES-valid.sh'
sh_dir = dirname(sh_file)
if (!dir.exists(sh_dir)) 
  dir.create(sh_dir, recursive = TRUE)
write_sh(job_name = 'calculate-outcomes-RNN-SELFIES-valid',
         sh_file = sh_file,
         grid_file = grid_file,
