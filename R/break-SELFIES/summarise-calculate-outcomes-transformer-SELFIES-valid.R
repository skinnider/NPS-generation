# Summarise outcomes from SELFIES models parsed with modified valency 
# constraints.
...
output_files = list.files(output_dir, pattern = 'outcomes-constraints', 
...
df = outputs %>% 
  ...
  type_convert() %>% 
  # flag constraints
  mutate(constraints = gsub("^.*constraints=|-valid.*$", "",
                            basename(input_file)))
...
# save
output_file = "data/summary/break-SELFIES/calculate-outcomes-transformer-SELFIES-valid.rds"
