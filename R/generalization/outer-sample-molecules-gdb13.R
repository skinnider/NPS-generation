...
# parse arguments
parser = ArgumentParser(prog = 'outer-sample-molecules-gdb13.R')
...
sample_dir = file.path(base_dir, "GDB13", "sample")
...
nvstat_files = with(grid, file.path(sample_dir, paste0(sample_dirnames, '.nvstat')))
...
sh_file = 'sh/sample-molecules/sample-molecules-gdb13.sh'
sh_dir = dirname(sh_file)
if (!dir.exists(sh_dir)) 
  dir.create(sh_dir, recursive = TRUE)
write_sh(job_name = 'sample-molecules-gdb13',
         sh_file = sh_file,
         grid_file = grid_file,
