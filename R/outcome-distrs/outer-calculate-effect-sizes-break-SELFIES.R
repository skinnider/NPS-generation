...
# property distributions of 'valid' vs. 'invalid' SELFIES.
setwd("~/git/invalid-smiles-analysis")
options(stringsAsFactors = FALSE)
library(argparse)

# parse arguments
parser = ArgumentParser(prog = 'outer-calculate-effect-sizes-break-SELFIES.R')
...
sample_dir = file.path(base_dir, "outcome_distrs", "break_SELFIES")
...
sample_filenames = with(grid, paste0('outcome-distrs-constraints=', constraints,
                                     '-', subset, '.csv.gz'))
sample_files = file.path(sample_dir, sample_dirnames, sample_filenames)
...
train_dirnames = pmap_chr(grid, function(...) {
  current = tibble(...)
  current %>%
    ## force SMILES and no enumeration
...
output_filenames = with(grid, paste0('effect-sizes-',
                                     'constraints=', constraints,
                                     '-', subset, '.rds'))
output_files = file.path(output_dir, sample_dirnames, output_filenames)
...
sh_file = 'sh/write-outcome-distrs/calculate-effect-sizes-break-SELFIES.sh'
sh_dir = dirname(sh_file)
if (!dir.exists(sh_dir)) 
  dir.create(sh_dir, recursive = TRUE)
write_sh(job_name = 'calculate-effect-sizes-break-SELFIES',
         sh_file = sh_file,
         grid_file = grid_file,
