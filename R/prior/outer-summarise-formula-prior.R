# First step of summarizing the formula prior:
# Compute % ever generated and top-k accuracy curves, summarizing over CV folds.
# Do this either: 
# - aggregating CV folds before summarizing
# - summarizing within CV folds then aggregating
setwd("~/git/invalid-smiles-analysis")
options(stringsAsFactors = FALSE)
library(argparse)

# parse arguments
parser = ArgumentParser(prog = 'outer-summarise-formula-prior.R')
...
output_dir = file.path(base_dir, "prior", "formula_prior")
...
output_files = file.path(output_dirs, 'top-k-accuracy.rds')
...
grid_file = "sh/grids/summarise-formula-prior.txt"
...
sh_file = 'sh/summarise-formula-prior.sh'
sh_dir = dirname(sh_file)
if (!dir.exists(sh_dir)) 
  dir.create(sh_dir, recursive = TRUE)
write_sh(job_name = 'summarise-formula-prior',
         sh_file = sh_file,
         grid_file = grid_file,
         inner_file = 'R/prior/inner-summarise-formula-prior.R',
         system = system,
         time = 8,
         mem = 48)
