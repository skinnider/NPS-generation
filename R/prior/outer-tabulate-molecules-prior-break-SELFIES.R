...
# parse arguments
parser = ArgumentParser(prog = 'outer-tabulate-molecules-prior-break-SELFIES.R')
...
grid_file = "sh/grids/tabulate-molecules-break-SELFIES.txt"
...
sh_file = 'sh/tabulate-molecules/tabulate-molecules-prior-break-SELFIES.sh'
sh_dir = dirname(sh_file)
if (!dir.exists(sh_dir)) 
  dir.create(sh_dir, recursive = TRUE)
write_sh(job_name = 'tabulate-molecules-prior-break-SELFIES',
         sh_file = sh_file,
         grid_file = grid_file,
         inner_file = 'python/inner-tabulate-molecules-break-SELFIES.py',
         system = system,
         time = 72,
         mem = 16)
