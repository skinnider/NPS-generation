...
# parse arguments
parser = ArgumentParser(prog = 'outer-write-pairwise-Tc-distribution.R')
...
output_dir = file.path(base_dir, "prior", "pairwise_Tc")
...
output_files = file.path(output_dirs, 'pairwise-Tc.csv.gz')
...
grid_file = "sh/grids/write-pairwise-Tc-distribution.txt"
...
sh_file = 'sh/write-pairwise-Tc-distribution.sh'
sh_dir = dirname(sh_file)
if (!dir.exists(sh_dir)) 
  dir.create(sh_dir, recursive = TRUE)
write_sh(job_name = 'write-pairwise-Tc-distribution',
         sh_file = sh_file,
         grid_file = grid_file,
         inner_file = 'python/inner-write-pairwise-Tc-distribution.py',
         system = system,
         time = 8,
         mem = 48)
