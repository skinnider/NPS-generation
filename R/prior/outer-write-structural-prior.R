...
# parse arguments
parser = ArgumentParser(prog = 'outer-write-structural-prior.R')
...
sample_files = file.path(sample_dir, sample_dirnames, 'unique-masses-novel.tsv')
...
output_files = file.path(output_dirs, 'CV-Tc.csv.gz')
...
grid_file = "sh/grids/write-structural-prior.txt"
...
sh_file = 'sh/prior/write-structural-prior.sh'
sh_dir = dirname(sh_file)
if (!dir.exists(sh_dir)) 
  dir.create(sh_dir, recursive = TRUE)
write_sh(job_name = 'prior-CV',
         sh_file = sh_file,
         grid_file = grid_file,
         inner_file = 'python/inner-write-structural-prior-CV.py',
         system = system,
         time = 48,
         mem = 96)
