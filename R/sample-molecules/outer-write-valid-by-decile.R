...
# parse arguments
parser = ArgumentParser(prog = 'outer-write-valid-by-decile.R')
...
# now, define output files: valid by decile
output_dir = file.path(base_dir, 'valid-by-decile')
output_filenames = paste0(basename(dirname(valid_files)), '__', 
                          basename(valid_files) %>% 
                            gsub("\\.csv", "-deciles.csv", .))
...
grid_file = "sh/grids/write-valid-by-decile.txt"
...
sh_file = 'sh/sample-molecules/write-valid-by-decile.sh'
sh_dir = dirname(sh_file)
if (!dir.exists(sh_dir)) 
  dir.create(sh_dir, recursive = TRUE)
write_sh(job_name = 'write-valid-by-decile',
         sh_file = sh_file,
         grid_file = grid_file,
         inner_file = 'R/sample-molecules/inner-write-valid-by-decile.R',
         system = system,
         time = 3,
         mem = 32)
