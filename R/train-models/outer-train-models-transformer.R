# Train transformer-based models of SMILES or SELFIES strings.
setwd("~/git/invalid-smiles-analysis")
options(stringsAsFactors = FALSE)
library(argparse)

# parse arguments
parser = ArgumentParser(prog = 'outer-train-models-transformer.R')
...
grid_file = "sh/grids/train-models-transformer.txt"
...
sh_file = 'sh/train-models/train-models-transformer.sh'
sh_dir = dirname(sh_file)
if (!dir.exists(sh_dir)) 
  dir.create(sh_dir, recursive = TRUE)
write_sh(job_name = 'train-models-transformer',
         sh_file = sh_file,
         grid_file = grid_file,
         inner_file = 'python/inner-train-models-transformer.py',
         system = system,
         time = 72,
         mem = 16,
         gpu = TRUE)
