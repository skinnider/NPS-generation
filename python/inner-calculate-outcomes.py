...
from functions import clean_mols, read_smiles, \
        ...
        pct_rotatable_bonds, pct_stereocenters
...
org_mols = [mol for mol in \
            clean_mols(org_smiles, representation=args.representation) if mol]
...
gen_mols = [mol for mol in \
            clean_mols(gen_smiles, representation=args.representation) if mol]
...
gen_mols = [gen_mols[idx] for idx, sm in enumerate(gen_canonical) if not \
            sm in org_set]
...
res = res.append(pd.DataFrame({
    'input_file': args.sampled_file, 
    'outcome': ['Jensen-Shannon distance, # of rings', 
...
res = res.append(pd.DataFrame({
    'input_file': args.sampled_file, 
    'outcome': ['Jensen-Shannon distance, hydrogen donors', 
