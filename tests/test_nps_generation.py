...
from NPS_generation.calculate_outcomes_distribution import (
    main as calculate_outcomes_distribution_main,
)
...
def test_clean_SMILES():
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        baseline_checksum = hashlib.md5(
            "".join(sorted(open(baseline_file, "r").readlines())).encode("utf8")
        ).hexdigest()
        output_checksum = hashlib.md5(
            "".join(sorted(open(output_file, "r").readlines())).encode("utf8")
        ).hexdigest()
